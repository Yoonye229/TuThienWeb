...
<?php
    ...
    if( $_SERVER["REQUEST_METHOD"] == "POST" ){
        ...
        if( empty($_REQUEST["p_category"])){
            $category_err =  "<p style='color:red'> * Vui lòng chọn chương trình </p>";
        }else {
          $category = $_REQUEST["p_category"];
        }

        if( empty( $_REQUEST["p_heading"] ) ){
            $p_heading_err = "<p style='color:red'> Vui lòng nhập tiêu đề </p>";
        }else{
            $p_heading = $_REQUEST["p_heading"];
        }

        if( empty( $_REQUEST["editor"] ) ){
            $editor_err = "<p style='color:red'> * Vui lòng nhập mô tả </p>";
        }else{
           $editor = $_REQUEST["editor"];
        }

        if( empty($_FILES["thumbnail"]["name"])){
           $thumbnail_err = "<p style='color:red'> * Vui lòng chọn ảnh </p>";
        }else{
        ...
        if($result_add_desc){
            ...
            $(document).ready( function(){
                ...
                $('#linkBtn').text('Thêm chi tiết bài báo');
                $('#addMsg').text('Mô tả đã được thêm vào!');
    ...
    <div id="form" class="pt-5 form-input-content">
        <div class="card login-form mb-0">
            <div class="card-body pt-3 shadow">
                <h4 class="text-center">Thêm mô tả bài viết </h4>
