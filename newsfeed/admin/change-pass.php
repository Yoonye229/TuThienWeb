...
<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($_REQUEST["old_pass"])){
            $old_passErr = " <p style='color:red'>* Yêu cầu mật khẩu cũ </p>";
        }else{
            $old_pass = trim($_REQUEST["old_pass"]);
        }
        
        if(empty($_REQUEST["new_pass"])){
            $new_passErr = " <p style='color:red'>* Yêu cầu mật khẩu mới </p>";
        }else{
            $new_pass = trim($_REQUEST["new_pass"]);
        }

        if(empty($_REQUEST["confirm_pass"])){
            $confirm_passErr = " <p style='color:red'>* VUi lòng nhập lại mật khẩu mới </p>";
        }else{
                ...
                if( $new_pass === $confirm_pass ){
                    ...
                    if (mysqli_query($conn , $change_pass_query) ){
                        ...
                        $(document).ready(function() {
                            $('#addMsg').text( 'Mật khẩu đã được thay đổi');
                            $('#linkBtn').attr('href','login.php');
                            $('#linkBtn').text('Thành công');
                            $('#modalHead').hide();
                            $('#closeBtn').hide();
                            ...
                            <div class="card-body pt-5 shadow">                       
                                    ...
                                    <form method="POST" action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>">
                                        ...
                                        <div class="btn-toolbar justify-content-between" role="toolbar" aria-label="Toolbar with button groups">
                                            <div class="btn-group">
                                        <input type="submit" value="Thay đổi" class="btn btn-primary w-20 " name="save_changes" >        
                                            </div>
